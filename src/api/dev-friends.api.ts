...
const allSquads = async () =>
  apiFetch<Squad[]>(`squads${isPlaywrightTest ? '' : '?delayMs=2000'}`);

const allDevs = async () =>
  apiPaginatedFetch<Dev[]>(`devs${isPlaywrightTest ? '' : '?delayMs=1000'}`);

const squadDevs = async (squadId: string) =>
  apiPaginatedFetch<Dev[]>(
    `squads/${squadId}/devs${isPlaywrightTest ? '' : '?delayMs=1000'}`,
  );
...
const devsBySquads = async (squadsIds: number[], page = 1) => {
  ...
  return apiPaginatedFetch<Dev[]>(
    `devs/by-squads${isPlaywrightTest ? '?' : '?delayMs=1000&'}page=${page}`,
    'POST',
    {
      idSquads: squadsIds,
    },
