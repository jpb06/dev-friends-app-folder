...
const allSquads = async () => apiFetch<Squad[]>('squads?delayMs=100');

const allDevs = async () => apiPaginatedFetch<Dev[]>('devs?delayMs=100');

const squadDevs = async (squadId: string) =>
  apiPaginatedFetch<Dev[]>(`squads/${squadId}/devs?delayMs=100`);
...
const devsBySquads = (squadsIds: number[], page = 1) => {
  ...
  return apiPaginatedFetch<Dev[]>(
    `devs/by-squads?delayMs=100&page=${page}`,
    'POST',
    {
      idSquads,
    },
